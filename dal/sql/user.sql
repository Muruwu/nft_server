...
(
    ...
    `certified_phone` VARCHAR(64) NULL comment '手机号',
...
create table user_promote_record
(
    `id` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT comment '主键id',
    `user_id` BIGINT(20) UNSIGNED NOT NULL comment '用户id',
    `source_user_id` BIGINT(20) UNSIGNED NOT NULL comment '推广来源用户id',
    `invite_code` VARCHAR(45) NULL comment '邀请码',
    `channel_code` VARCHAR(20) NULL comment '渠道邀请码',
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP comment '创建时间',
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP comment '更新时间',
  ...
  ) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 comment '拉新推广流水表';
